...
class DarknetCfgToPytorchModuleConverter:
    def convert(self, cfg_path, python_output_path, class_name):
        layers = self._read_layers(cfg_path)
        with open(python_output_path, 'w') as python_file:
            in_channels = self._write_header(python_file, layers[0], class_name, cfg_path)
            self._write_init(python_file, layers[1:], in_channels, class_name)
            self._write_getters(python_file)
            self._write_forward(python_file, layers[1:])
            self._write_load_weights(python_file, layers[1:])
            self._write_load_batch_norm_conv_weights(python_file)
            self._write_load_conv_weights(python_file)

    def _read_layers(self, cfg_path):
    ...
    def _write_header(self, python_file, layer, class_name, cfg_path):
        if layer['type'] != 'net':
    ...
    def _write_init(self, python_file, layers, in_channels, class_name):
        ...
        for i, layer in enumerate(layers):
            if layer['type'] == 'convolutional':
                out_channels, stride = self._write_init_convolutional(python_file, i, layer, in_channels[-1])
                in_channels.append(out_channels)
                cumulated_strides.append(cumulated_strides[-1] * stride)
            elif layer['type'] == 'upsample':
                out_channels, stride = self._write_init_upsample(python_file, i, layer, in_channels[-1])
                in_channels.append(out_channels)
                cumulated_strides.append(cumulated_strides[-1] // stride)
            elif layer['type'] == 'maxpool':
                out_channels, stride = self._write_init_maxpool(python_file, i, layer, in_channels[-1])
                in_channels.append(out_channels)
                cumulated_strides.append(cumulated_strides[-1] * stride)
            elif layer['type'] == 'yolo':
                in_channels.append(self._write_init_yolo(python_file, i, layer, cumulated_strides[-1]))
                cumulated_strides.append(0)
            elif layer['type'] == 'route':
                out_channels, stride = self._write_init_route(layer, in_channels, cumulated_strides)
                in_channels.append(out_channels)
                cumulated_strides.append(stride)
            elif layer['type'] == 'shortcut':
                out_channels, stride = self._write_init_shortcut(layer, in_channels, cumulated_strides)
                in_channels.append(out_channels)
                cumulated_strides.append(stride)
            else:
    ...
    def _write_init_convolutional(self, python_file, i, layer, in_channels):
    ...
    def _write_init_upsample(self, python_file, i, layer, in_channels):
    ...
    def _write_init_maxpool(self, python_file, i, layer, in_channels):
    ...
    def _write_init_route(self, layer, in_channels, cumulated_strides):
    ...
    def _write_init_shortcut(self, layer, in_channels, cumulated_strides):
    ...
    def _write_init_yolo(self, python_file, i, layer, cumulated_stride):
        ...
        python_file.write('        self._yolo{} = YoloLayer({}, {}, {}, {}, {})'
                          .format(i, 'IMAGE_SIZE', cumulated_stride, 'self._anchors[-1].tolist()', class_count,
                                  scale_x_y))
        return 0

    def _write_getters(self, python_file):
    ...
    def _write_forward(self, python_file, layers):
        ...
        for i, layer in enumerate(layers):
            ...
            if layer['type'] == 'convolutional':
                ...
                output_name = self._write_forward_route(python_file, i, layer, output_names, output_name)
            elif layer['type'] == 'shortcut':
                from_ = int(layer['from'])
    ...
    def _write_forward_route(self, python_file, i, layer, output_names, output_name):
    ...
    def _write_load_weights(self, python_file, layers):
    ...
    def _write_load_batch_norm_conv_weights(self, python_file):
    ...
    def _write_load_conv_weights(self, python_file):
...
def main():
    ...
    converter = DarknetCfgToPytorchModuleConverter()
    converter.convert(args.cfg_path, args.python_output_path, args.class_name)
