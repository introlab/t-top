...
AVAILABLE_CLASS_NAMES = ['Apple', 'Artichoke', 'Bagel', 'Banana', 'Band-aid', 'Beer', 'Bell pepper', 'Book', 'Bottle',
                         'Bottle opener', 'Bowl', 'Bread', 'Broccoli', 'Burrito', 'Cabbage', 'Cake', 'Cantaloupe',
                         'Carrot', 'Cheese', 'Chopsticks', 'Cocktail shaker', 'Coffee cup', 'Common fig',
                         'Cookie', 'Croissant', 'Cucumber', 'Doughnut', 'Drinking straw', 'Food processor',
                         'Frying pan', 'Garden Asparagus', 'Grape', 'Grapefruit', 'Guacamole', 'Hamburger', 'Hot dog',
                         'Juice', 'Kitchen knife', 'Lemon', 'Mango', 'Milk', 'Mixing bowl', 'Orange', 'Pancake',
                         'Person', 'Pizza', 'Pizza cutter', 'Pomegranate', 'Popcorn', 'Potato', 'Pressure cooker',
                         'Pretzel', 'Sandwich', 'Submarine sandwich', 'Tomato', 'Waffle', 'Wine', 'Winter melon',
                         'Zucchini']
AVAILABLE_CLASS_IDS = ['/m/014j1m', '/m/047v4b', '/m/01fb_0', '/m/09qck', '/m/01599', '/m/0jg57', '/m/09728',
                       ...
                       '/m/02cvgx', '/m/027pcv', '/m/0j496', '/m/0bt_c3', '/m/04dr76w', '/m/04f5ws', '/m/04kkgm',
                       '/m/01_5g', '/m/0440zs', '/m/03v5tg', '/m/03y6mg', '/m/04v6l4', '/m/01g317', '/m/058qzx',
                       '/m/03hj559', '/m/08ks85', '/m/0h8ntjv']
CLASS_COUNT = len(AVAILABLE_CLASS_IDS) + 1


# background class index = 0
class SemanticSegmentationOpenImages(OpenImages):
    def __init__(self, root, split=None, transforms=None):
        super(SemanticSegmentationOpenImages, self).__init__(root, split=split, transforms=transforms)

        if len(self._indexes_by_class_id) != CLASS_COUNT:
            raise ValueError('Invalid dataset root')

    def _list_available_class_ids(self, root):
        return set(AVAILABLE_CLASS_IDS)
