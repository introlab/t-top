...
class FaceRecognitionFolderDataset(Dataset):
    def __init__(self, path, transforms=None, ignored_classes=None):
        if ignored_classes is None:
            ignored_classes = []

        self._class_names = [o for o in os.listdir(path) if os.path.isdir(os.path.join(path, o))]
        self._class_names = list(set(self._class_names) - set(ignored_classes))
        self._class_names.sort()

        self._all_images, self._images_by_class = self._list_images(path)

        self._transforms = transforms

    def _list_images(self, train_path):
        paths_by_class = []
        all_paths = []
        ...
        for i in range(len(self._class_names)):
            ...
            for image in os.listdir(class_path):
                ...
                all_paths.append({'path': path, 'class': i})

                paths.append({'path': path, 'index': len(all_paths) - 1})
            paths_by_class.append(paths)

        return all_paths, paths_by_class
    ...
    def __getitem__(self, index):
        ...
        return image, self._all_images[index]['class']
