...
from dnn_utils import DescriptorYoloV4, PoseEstimator, FaceDescriptorExtractor, AudioDescriptorExtractor, VoiceDescriptorExtractor, TTopKeywordSpotter
...
def test_descriptor_yolo_v4():
    ...
    _, cpu_predictions =  cpu_model.forward_raw_cpu(x)
    _, torch_gpu_predictions = torch_gpu_model.forward_raw_cpu(x)
    _, trt_gpu_predictions = trt_gpu_model.forward_raw_cpu(x)
...
def test_pose_estimator():
    ...
    IMAGE_SIZE = cpu_model.get_supported_image_size()
    x = to_pil_image(torch.rand(3, IMAGE_SIZE[0], IMAGE_SIZE[1]))
...
def test_face_descriptor_extractor():
    ...
    cpu_descriptor = cpu_model(x, pose_coordinates, pose_presence)
    torch_gpu_descriptor = torch_gpu_model(x, pose_coordinates, pose_presence)
    trt_gpu_descriptor = trt_gpu_model(x, pose_coordinates, pose_presence)
