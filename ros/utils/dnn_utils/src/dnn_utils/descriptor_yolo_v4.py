...
class DescriptorYoloV4Prediction:
...
class DescriptorYoloV4(DnnModel):
    def __init__(self, confidence_threshold=0.99, nms_threshold=0.5, inference_type=None):
        self._confidence_threshold = confidence_threshold
        self._nms_threshold = nms_threshold

        torch_script_model_path = os.path.join(PACKAGE_PATH, 'models', 'descriptor_yolo_v4.ts.pth')
        tensor_rt_model_path = os.path.join(PACKAGE_PATH, 'models', 'descriptor_yolo_v4.trt.pth')
        sample_input = torch.ones(1, 3, IMAGE_SIZE[0], IMAGE_SIZE[1])

        super(DescriptorYoloV4, self).__init__(torch_script_model_path, tensor_rt_model_path, sample_input,
    ...
    def __call__(self, image_tensor):
        with torch.no_grad():
            ...
            return [DescriptorYoloV4Prediction(prediction.cpu(), scale) for prediction in predictions]

    def forward_raw(self, image_tensor):
        scale = self._set_image(image_tensor.to(self._device).unsqueeze(0))
        predictions = super(DescriptorYoloV4, self).__call__(self._padded_image)
