...
class PoseEstimatorBest(DnnModel):
    def __init__(self, inference_type=None):
        torch_script_model_path = os.path.join(PACKAGE_PATH, 'models', 'pose_estimator_best.ts.pth')
        tensor_rt_model_path = os.path.join(PACKAGE_PATH, 'models', 'pose_estimator_best.trt.pth')
        sample_input = torch.ones((1, 3, IMAGE_SIZE[0], IMAGE_SIZE[1]))

        super(PoseEstimatorBest, self).__init__(torch_script_model_path, tensor_rt_model_path, sample_input,
