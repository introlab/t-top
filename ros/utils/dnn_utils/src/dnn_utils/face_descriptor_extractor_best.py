...
class FaceDescriptorExtractorBest(DnnModel):
    def __init__(self, inference_type=None):
        torch_script_model_path = os.path.join(PACKAGE_PATH, 'models', 'face_descriptor_extractor_best.ts.pth')
        tensor_rt_model_path = os.path.join(PACKAGE_PATH, 'models', 'face_descriptor_extractor_best.trt.pth')
        sample_input = torch.ones((1, 3, IMAGE_SIZE[0], IMAGE_SIZE[1]))

        super(FaceDescriptorExtractorBest, self).__init__(torch_script_model_path, tensor_rt_model_path, sample_input,
