...
class MulticlassAudioDescriptorExtractorBest(DnnModel):
    def __init__(self, inference_type=None):
        torch_script_model_path = os.path.join(PACKAGE_PATH, 'models', 'multiclass_audio_descriptor_extractor_best.ts.pth')
    ...
    def __call__(self, x):
        with torch.no_grad():
            ...
            class_scores = super(MulticlassAudioDescriptorExtractorBest, self).__call__(spectrogram.unsqueeze(0))
            probabilities = torch.sigmoid(class_scores[0])

            return torch.zeros(128), probabilities.cpu()
