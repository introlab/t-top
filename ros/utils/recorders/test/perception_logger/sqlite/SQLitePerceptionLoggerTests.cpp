...
TEST(SQLitePerceptionLoggerTests, insertPerception_shouldInsertAndReturnId)
{
    ...
    int64_t id0 = logger.insertPerception(Timestamp(101), std::nullopt, std::nullopt);
    int64_t id1 = logger.insertPerception(Timestamp(102), Position{10, 11, 12}, std::nullopt);
    int64_t id2 = logger.insertPerception(Timestamp(103), std::nullopt, Direction{20, 21, 22});
