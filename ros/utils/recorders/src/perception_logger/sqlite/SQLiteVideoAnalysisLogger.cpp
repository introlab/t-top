...
int64_t SQLiteVideoAnalysisLogger::log(const VideoAnalysis& analysis)
{
    ...
    {
        insert.bind(3, reinterpret_cast<const void*>(analysis.personPose.value().data()), analysis.personPose.value().size() * sizeof(Position));
    }
    if (analysis.personPoseConfidence.has_value())
    {
        insert.bind(4, reinterpret_cast<const void*>(analysis.personPoseConfidence.value().data()), analysis.personPoseConfidence.value().size() * sizeof(float));
    }
    if (analysis.faceDescriptor.has_value())
    {
        insert.bind(5, reinterpret_cast<const void*>(analysis.faceDescriptor.value().data()), analysis.faceDescriptor.value().size() * sizeof(float));
    }
