...
public:
    template<class T>
    HbbaSubscriber(ros::NodeHandle& nodeHandle,
            const std::string& topic, uint32_t queueSize, void(T::*fp)(const typename MessageType::ConstPtr&), T* obj,
            const std::string& stateServiceName = "", const ros::TransportHints& transportHints = ros::TransportHints());
    HbbaSubscriber(ros::NodeHandle& nodeHandle,
            const std::string& topic, uint32_t queueSize, void(*fp)(const typename MessageType::ConstPtr&),
            const std::string& stateServiceName = "", const ros::TransportHints& transportHints = ros::TransportHints());
...
template <class FilterState, class MessageType>
template<class T>
HbbaSubscriber<FilterState, MessageType>::HbbaSubscriber(ros::NodeHandle& nodeHandle,
        const std::string& topic, uint32_t queueSize, void(T::*fp)(const typename MessageType::ConstPtr&), T* obj,
        const std::string& stateServiceName, const ros::TransportHints& transportHints) :
    m_filterState(nodeHandle, stateServiceName == "" ? topic + "/filter_state" : stateServiceName)
{
    m_callback = boost::bind(fp, obj, _1);
    m_subscriber = nodeHandle.subscribe(topic, queueSize,
            &HbbaSubscriber<FilterState, MessageType>::callback, this, transportHints);
}
