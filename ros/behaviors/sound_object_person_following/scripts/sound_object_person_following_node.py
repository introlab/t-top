...
class ObjectPersonFollower(Follower):
    ...
    def _video_analysis_cb(self, msg):
        ...
        if person is None:
            ...
            target = self._find_target(msg.semantic_segmentation[0], person)
    ...
    def _find_target(self, semantic_segmentation, person, eps=1e-6):
        min_x, max_x, min_y, max_y = self._find_target_range(person)
        mask = self._get_mask_from_semantic_segmentation(semantic_segmentation)
