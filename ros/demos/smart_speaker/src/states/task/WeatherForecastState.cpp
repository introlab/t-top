    ...
    : TalkState(language, stateManager, desireSet, move(node), nextStateType)
{
    ...
    m_weatherClient = m_node->create_client<cloud_data::srv::LocalWeatherForecastOpenMeteo>(
...
{
    bool ok;
    cloud_data::srv::LocalWeatherForecastOpenMeteo::Response response;
    ...
    if (ok)
    {
        ...
        ss << "Tomorrow, the precipitation probability will be " << response.precipitation_probability_percent << "%, ";
...
string WeatherForecastState::generateFrenchText([[maybe_unused]] const string& _)
{
    ...
    if (ok)
    {
        ...
        ss << "Demain, la probabilité de précipitation sera de " << response.precipitation_probability_percent << "%, ";
...
void WeatherForecastState::getLocalWeatherForecast(bool& ok, cloud_data::srv::LocalWeatherForecastOpenMeteo::Response& response)
{
    auto request = make_shared<cloud_data::srv::LocalWeatherForecastOpenMeteo::Request>();
