...
class VideoAnalyzerNode(rclpy.node.Node):
    def __init__(self, node_name):
        super().__init__(node_name)

        self._use_descriptor_yolo = self.declare_parameter('use_descriptor_yolo', False).get_parameter_value().bool_value
        self._yolo_model = self.declare_parameter('yolo_model', 'yolo_v7_coco').get_parameter_value().string_value
        self._confidence_threshold = self.declare_parameter('confidence_threshold', 0.5).get_parameter_value().double_value
        self._nms_threshold = self.declare_parameter('nms_threshold', 0.5).get_parameter_value().double_value
        self._person_probability_threshold = self.declare_parameter('person_probability_threshold', 0.5).get_parameter_value().double_value
        ...
        self._pose_enabled = self.declare_parameter('pose_enabled', True).get_parameter_value().bool_value
        self._face_descriptor_enabled = self.declare_parameter('face_descriptor_enabled', True).get_parameter_value().bool_value
        self._semantic_segmentation_enabled = self.declare_parameter('semantic_segmentation_enabled', False).get_parameter_value().bool_value
        self._semantic_segmentation_dataset = self.declare_parameter('semantic_segmentation_dataset', 'coco').get_parameter_value().string_value
        self._cropped_image_enabled = self.declare_parameter('cropped_image_enabled', False).get_parameter_value().bool_value
        ...
        self._video_analysis_pub = self.create_publisher(VideoAnalysis, 'video_analysis', 10)
        self._analysed_image_pub = hbba_lite.OnOffHbbaPublisher(self, Image, 'analysed_image', 10)
