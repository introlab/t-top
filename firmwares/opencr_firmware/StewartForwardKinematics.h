...
public:
  ...
  void calculatePose(Eigen::Vector3f& position,
...
private:
  ...
  void solverFunction(Eigen::Matrix<float, SOLVER_X_SIZE, 1>& f,
      Eigen::Matrix<float, SOLVER_X_SIZE, SOLVER_X_SIZE>& J,
      const Eigen::Matrix<float, SOLVER_X_SIZE, 1>& x);
  void calculateRodLengthErrorAndGrad(float& error, Eigen::Matrix<float, SOLVER_X_SIZE, 1>& grad,
