...
public:
  ...
  bool writeChargeCurrentLimitToRam(float current, size_t trialCount = RRC_DEFAULT_TRIAL_COUNT); // Write the charge current limit to RAM. The current unit is A.
  bool writeChargeCurrentLimitToEeprom(float current, size_t trialCount = RRC_DEFAULT_TRIAL_COUNT); // Write the charge current limit to EEPROM. The current unit is A.

  bool writeInputCurrentLimitToRam(float current, size_t trialCount = RRC_DEFAULT_TRIAL_COUNT); // Write the input current limit to RAM. The current unit is A.
  bool writeInputCurrentLimitToEeprom(float current, size_t trialCount = RRC_DEFAULT_TRIAL_COUNT); // Write the input current limit to EEPROM. The current unit is A.
